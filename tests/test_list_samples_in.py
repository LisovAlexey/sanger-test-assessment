...
def well_one(sample_one, plate_barcode, database_layer):
    yield database_layer.add_to_plate(sample_id=sample_one.sample_id,
...
class TestListSamplesIn:
    ...
    def test_list_samples_in_tube(self, database_layer, sample_one):
        ...
        assert tube_report.barcode == self.sample_one.tube_barcode
        assert tube_report.sample_id == self.sample_one.id
        assert tube_report.customer_sample_name == self.sample_one.customer_sample_name
    ...
    def test_list_samples_in_plate(self, database_layer, plate_barcode, wells):
        ...
        assert plate_report.barcode == self.plate_barcode
        assert set(plate_report.wells) == set(wells)
        assert plate_report.customer_sample_name == sample_one.customer_sample_name
